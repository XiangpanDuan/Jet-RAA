	...
	for(int ix=0; ix<_xnum; ix++){
		for(int iy=0; iy<_ynum; iy++){
			for(int itau=0; itau<_taunum; itau++){
				...
				_Temp[ix][iy][itau]=tempval; //attention x,y,tau
        // if(xval==0 && yval==0 && tauval==0.6) std::cout << ix << "  " << iy << "  " << itau << std::endl; //test zero point 
			}
		}
	}
  InputFile.close();
  int xhalf=floor(_xnum/2);
  int yhalf=floor(_ynum/2);
...
double TrilinearInterpolation::TriInterpolation(const double xin, const double yin, const double tauin){
  ...
  xlow=floor(xin/_xbin); xhigh=xlow+1;
	ylow=floor(yin/_ybin); yhigh=ylow+1;
	taulow=floor(tauin/_taubin); tauhigh=taulow+1; //attention
...
double TrilinearInterpolation::getOmegaC(){
  return _omegaC;
}
...
//*************************************************************************************************************************************************************************
//MC integration
void TrilinearInterpolation::setupMC(double (*pds)(double *, size_t, void *), size_t dim, double *rangemin, double *rangemax, void *para){
...
void TrilinearInterpolation::cleanupMC(){
#ifdef MC_MISER
...
void TrilinearInterpolation::setCalls(size_t calls){
  _calls=calls;
}


void TrilinearInterpolation::calculate(double &res, double &err){
#ifdef MC_MISER
  gsl_monte_miser_integrate (&_mcFun, _rangemin, _rangemax, _dim, _calls, _r, _mcStatus, &res, &err);
#else
  gsl_monte_vegas_integrate (&_mcFun, _rangemin, _rangemax, _dim, _calls, _r, _mcStatus, &res, &err);
#endif
}