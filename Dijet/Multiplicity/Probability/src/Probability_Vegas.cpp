...
int    ival,jval;
double Rsize=0.4;
double Qmed=10.0;  //50->15; 33->12; 25->10;
...
//------------------------------------------------------------
double Function(double yp[], size_t dim, void *params){
  (void) (dim);
  (void) (params);
  double gamma0square=4.*Nc/(b*(yp[0]+lambda));
  double Pval=LinearInterpolation(yp[0]);
  int    nFactorial=Factorial(jval+1);
  // std::cout << yp[0] << " " << Pval << " " << (ymax[0]-yp[0])*gamma0square*Pval << std::endl;
  return (ymax[0]-yp[0])*gamma0square*Pval*nFactorial;
}

//------------------------------------------------------------
double FunctionBase(double yp[], size_t dim, void *params){
  (void) (dim);
  (void) (params);
  double gamma0square=4.*Nc/(b*(yp[0]+lambda));
  // std::cout << ymax[0]-yp[0] << "  " << (ymax[0]-yp)*gamma0square << std::endl;
  return (ymax[0]-yp[0])*gamma0square;
}
...
void CalProbability(){
  ...
  std::stringstream ss;
  ss << "../Output/Vegas_Probability_Qmed" << Qmed << ".dat";
  ...
  for(int i=0; i<pTnum; i++){
    ...
    for(int j=0; j<Pnum; j++){
      ...
      if(j==0){
        for(int k=0; k<num; k++){
          ymin[0]=0.0;
          ymax[0]=ybin*(k+1);  //update current y value
          //MC integration
          size_t calls = 50000;
          size_t dim = 1;
          const gsl_rng_type *T;
          gsl_rng *r;
          // gsl_monte_function FunBase = {&FunctionBase, dim, 0};
          gsl_rng_env_setup();
          T = gsl_rng_default;
          r = gsl_rng_alloc(T);
          gsl_monte_function FunBase;
          FunBase.f = &FunctionBase;
          FunBase.dim = dim;
          FunBase.params = nullptr;
          gsl_monte_vegas_state *Status = gsl_monte_vegas_alloc(dim);
          res=0.0; err=0.0;
          gsl_monte_vegas_integrate(&FunBase, ymin, ymax, dim, calls, r, Status, &res, &err);
          gsl_rng_free(r);
          gsl_monte_vegas_free(Status);
      ...
      for(int k=0; k<num; k++){
        ...
        const gsl_rng_type *T;
        gsl_rng *r;
        // gsl_monte_function Fun = {&Function, dim, 0};
        gsl_rng_env_setup();
        T = gsl_rng_default;
        r = gsl_rng_alloc(T);
        ...
        gsl_monte_vegas_state *Status = gsl_monte_vegas_alloc(dim);
        res=0.0; err=0.0;
        gsl_monte_vegas_integrate(&Fun, ymin, ymax, dim, calls, r, Status, &res, &err);
        gsl_rng_free(r);
        gsl_monte_vegas_free(Status);
        SuTable[i][j][k]=res;
      }
      ...
      if(j==0){
        ymin[0]=0.0;
        ymax[0]=std::log(pTbinmax/Qmed);
        //Vegas integration
        size_t calls = 50000;
        size_t dim = 1;
        const gsl_rng_type *T;
        gsl_rng *r;
        // gsl_monte_function FBase = {&FunctionBase, dim, 0};
        gsl_rng_env_setup();
        T = gsl_rng_default;
        r = gsl_rng_alloc(T);
        gsl_monte_function FBase;
        FBase.f = &FunctionBase;
        FBase.dim = dim;
        FBase.params = nullptr;
        gsl_monte_vegas_state *Status = gsl_monte_vegas_alloc(dim);
        res=0.0; err=0.0;
        gsl_monte_vegas_integrate(&FBase, ymin, ymax, dim, calls, r, Status, &res, &err);
        gsl_rng_free(r);
        gsl_monte_vegas_free(Status);
        P[i][j]=exp(-res);
      }
      ...
      const gsl_rng_type *T;
      gsl_rng *r;
      // gsl_monte_function F = {&Function, dim, 0};
      gsl_rng_env_setup();
      T = gsl_rng_default;
      r = gsl_rng_alloc(T);
      ...
      gsl_monte_vegas_state *Status = gsl_monte_vegas_alloc(dim);
      res=0.0; err=0.0;
      gsl_monte_vegas_integrate(&F, ymin, ymax, dim, calls, r, Status, &res, &err);
      gsl_rng_free(r);
      gsl_monte_vegas_free(Status);
      Su[i][j]=res;
